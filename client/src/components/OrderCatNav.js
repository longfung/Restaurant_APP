...
function OrderCatNav(props) {
  ...
  const { setTopCategory, topCategory, topCategoryList } = props;
  debugger;
  ...
  return (
    <div className={classes.root}>
      <AppBar position="static" color="primary">
        <Toolbar >
          {topCategoryList && topCategoryList.length > 100 ?
            <Grid container direction="row">
              <Grid item xs={3} >
                <FormControl variant="outlined" className={classes.root}>
                  ...
                  >
                    {topCategoryList && topCategoryList.map(option => (
            ...
            //       onClick={() => setTopCategory(option.value)} >
            //       {option.label}
            //     </Button>
            //   </Grid>
            // ))
            ...
            <Tabs value={tabValue} onChange={handleChange} variant="scrollable" scrollButtons="auto">
              {topCategoryList && topCategoryList.map((option, idx) => (
                <Tab label={option.label} {...a11yProps(idx)} />
              ))}
            </Tabs>
